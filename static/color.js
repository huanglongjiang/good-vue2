    ...
    Color.prototype.toString = function(style) {
        ...
        case 'hsl':
            ...
            if (this.hsl.length == 3) {
                str = 'hsl(' + this.hsl[0] + 'deg,' + this.hsl[1] + '%,' + this.hsl[2] + '%)';
            }
            break;
        case 'rgb':
        default:
...
function setColor(){
    ...
    if(getColor!=null){
        ...
        function hslFn(hsl){
          let val=hsl.split(',')[2].split('%')[0]
          let hsl2=`${hsl.split(',')[0]},${hsl.split(',')[1]},${val-5}%)`
          return hsl2
        }

        let hsl=convertColor(getColor)
        let hsl2=hslFn(hsl);
        let hsl3=hslFn(hslFn(hsl));
